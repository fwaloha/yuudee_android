...
public class PermissionUtil {
     ...
     * @throws RuntimeException if not in the MainThread
     */
//    public void request(MyApplication_host myApplication_host, @NonNull String[] permissions, PermissionResultCallBack callBack) {
//
//        if (!this.checkSituation(permissions, callBack)) {
//            return;
//        }
...
//    }
    ...
    private void request(Activity activity, String[] permissions) {
        Map<String, List<PermissionInfo>> map = PermissionUtil.getInstance().checkMultiPermissions(permissions);
        List<PermissionInfo> dataList = map.get(DENIED);
        if(dataList != null && dataList.size() > 0){
            ToastUtil.showToast(activity,"访问权限已禁止,请去应用设置打开");
        }
...
//        }
    ...
    public void checkCommonPermissions(Activity activity)
    {
        PermissionUtil.getInstance().request(activity, new String[]{
