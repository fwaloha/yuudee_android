...
public class MyUtils {
    ...
    public static void goToLogin(Context context){
        ...
        ToastUtil.showToast(context, Constants.MISSTOKEN);
    }
    public static boolean CheckPhoneNum(Activity mContext, String phoneNum) {
        if (TextUtils.isEmpty(phoneNum)) {
//            ToastUtil.showToast(mContext, "手机号不能为空");
            return false;
        }
        if (phoneNum.length() != 11) {
//            ToastUtil.showToast(mContext, "手机号位数不是11位");
            return false;
        }

        Boolean myMobileNO = TelNumMatch.isMobileNO(phoneNum);
        if (!myMobileNO) {
//            ToastUtil.showToast(mContext, "请输入有效的手机号");
            return false;
        }
        return true;
    }
